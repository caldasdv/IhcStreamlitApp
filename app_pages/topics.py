...
try:
    topics = services.topics.list_for_subject(user["_id"], user.get("current_academic_period_id"), selected_subject_id)
...
with st.expander("Adicionar tópico ou subtópico", expanded=not topics):
    ...
    if submitted:
        try:
            services.topics.create(user_id=user["_id"], academic_period_id=user.get("current_academic_period_id"), subject_id=selected_subject_id, parent_id=parent_id, title=title, status=status, difficulty=difficulty)
